...
{
    // https://cnbin.github.io/blog/2016/06/07/ioskai-fa-zhong-de-huo-xing-zuo-biao-xi-ji-ge-chong-zuo-biao-xi-zhuan-huan-suan-fa/
    
    double gLat = self.coordinate.latitude;
    double gLon = self.coordinate.longitude;
    CLLocationCoordinate2D marsCoor = self.locationMarsFromEarth.coordinate;
    double dLat = marsCoor.latitude - gLat;
    double dLon = marsCoor.longitude - gLon;
    CLLocationCoordinate2D earthCoor = CLLocationCoordinate2DMake(gLat - dLat, gLon - dLon);
    
    return [[CLLocation alloc] initWithCoordinate:earthCoor
                                         altitude:self.altitude
                               horizontalAccuracy:self.horizontalAccuracy
                                 verticalAccuracy:self.verticalAccuracy
                                           course:self.course
